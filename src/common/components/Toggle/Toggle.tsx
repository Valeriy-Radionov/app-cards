...
export const Toggle = (props: TogglePropsType) => {
    ...
    return (
        <div className={style.container}>
            <div onClick={(e) => switchItem(e)} data-toggle={'first'}
                 className={`${style.item} ${classNameFirstItem}`}>My
            </div>
            <div onClick={switchItem} data-toggle={'second'}
                 className={`${style.item} ${classNameSecondItem}`}>All
            </div>
        </div>
    );
};